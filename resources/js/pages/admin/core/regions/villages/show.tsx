                ...
                <div className="border-sidebar-border/70 dark:border-sidebar-border relative min-h-screen flex-1 overflow-hidden rounded-xl border md:min-h-min">
                    <div className="grid grid-cols-1 gap-4 p-4 md:grid-cols-2 md:p-6">
                        ...
                        <div className="flex flex-col space-y-2">
                            ...
                            <p className="text-sm">{village.district.name}</p>
                        </div>
                        <div className="flex flex-col space-y-2">
                            <label htmlFor="email" className="text-sm font-semibold">
                                Created At
                            </label>
                            <p className="text-sm">{moment(village.created_at).tz('Asia/Jakarta').format('DD MMMM YYYY')}</p>
                        </div>
                        <div className="flex flex-col space-y-2">
                            <label htmlFor="email" className="text-sm font-semibold">
                                Updated At
                            </label>
                            <p className="text-sm">{moment(village.updated_at).tz('Asia/Jakarta').format('DD MMMM YYYY')}</p>
                        </div>
                    </div>
                </div>
            </div>
